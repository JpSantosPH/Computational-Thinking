...
index_of_letter('j'), index_of_letter('q'), index_of_letter('z')
...
# ╔═╡ 5531a72d-b590-48c0-8537-d1519b2cc2ea
...
# ╔═╡ 32312897-a8b7-47e0-ac35-bc5051cceda6
typeof(sample_freq_matrix)

# ╔═╡ 64476cc0-0a7c-41e9-a5eb-53d80d20e629
function double_letter(x::Matrix)
	a, b = size(x)

	c = AbstractVector{Char}([])
	for i in 1:a
		for j in 1:b
			if i == j && x[i, j] != 0
			push!(c, alphabet[i])
...
double_letters = double_letter(sample_freq_matrix) # replace with your answer
...
# ╔═╡ 0f44dfc8-c77b-4d41-a6e3-195453158113
findmax(sample_freq_matrix[23, :])

# ╔═╡ 7898b76a-f930-11ea-2b7e-8126ec2b8ffd
most_likely_to_follow_w = alphabet[15] # replace with your answer
...
# ╔═╡ 6500f893-5e5f-41ab-91c1-a56bd8c97efc
findmax(sample_freq_matrix[:, 23])
...
row_sums = [sum(sample_freq_matrix[i, :]) for i ∈ 1:27]

# ╔═╡ 4a0314a6-7dc0-4ee9-842b-3f9bd4d61fb1
col_sums = [sum(sample_freq_matrix[:, i]) for i ∈ 1:27]

# ╔═╡ a9acb51c-4577-46d0-aa08-08dfe074eb06
matrix_sums = sum(sample_freq_matrix)

# ╔═╡ cc62929e-f9af-11ea-06b9-439ac08dcb52
row_col_answer = md"""

These values can be interpreted as the structure of an language.
"""
...
function matrix_distance(A, B)
	D = A .- B
	D = abs.(D)
	return sum(D) # do something with A .- B
end
...
# ╔═╡ 8f647db6-72e1-4bb8-af81-e453210d1c8c
bigrams([1, 2, 3, 42])
...
# ╔═╡ ec571f34-59ed-4d9a-baaf-2dace8a9e0cc
aa = Dict(
	"to" => 2, 
	"be" => 2, 
	"or" => 1, 
...
# ╔═╡ 20171668-71ae-4177-b56d-72312c8e3ec5
emma_word_count = word_counts(emma_words)

# ╔═╡ 953363dc-fb84-11ea-1128-ebdfaf5160ee
emma_count = emma_word_count["Emma"]
...
# ╔═╡ 2528e4c9-30b1-4323-93b4-5982d1ff3dc9
typemma_words

# ╔═╡ 68f28845-f7dd-435a-92f2-b009f043e4db
ngrams(emma_words, 3)[1][1:2]

# ╔═╡ eed777cf-f13d-4000-a60d-fa6c3ecf689c
...
function completion_cache(grams)
	...
	for i in grams
		if haskey(cache, i[1:2])
			push!(cache[i[1:2]], i[3])
		else
			push!(cache, i[1:2]=>[i[3]])
		end
	end
...
# ╔═╡ 5c03dce6-ae85-4c48-8d8b-400231621701
trigrams = ngrams(emma_words, 3)

# ╔═╡ 8e9e7302-645b-4dc0-8050-1ff0445527b1
