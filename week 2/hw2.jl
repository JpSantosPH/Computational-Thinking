...
Submission by: **_$(John Paul)_** ($(student.kerberos_id)@mit.edu)
"""
...
function extend(M::AbstractMatrix, i, j)
	y, x = size(M)
	if i in 1:y && j in 1:x
		return M[i, j]
	elseif i in 1:y && !(j in 1:x)
		if j < 1
			return M[i, 1]
		elseif j > x
			return M[i, x]
...
function box_blur(v::AbstractArray, l)
	...
	for i in 1:a
		accumulator = []
		for j in i-l:i+l
			push!(accumulator, extend(v, j))
		end
		b[i] = mean(accumulator)
	end
	return b
end
...
function convolve(v::AbstractVector, k)
	...
	for i in 1:a
		...
		for j in -b_half:b_half
			z = extend(v, i+j) * k[j+b_half+1]
			accumulator += z
		end
...
# ╔═╡ 505a4e9d-027e-4c04-a46f-4c48ef360048
size(philip_head)
...
function convolve(M::AbstractMatrix, K::AbstractMatrix)
	...
	for i in 1:a
		for j in 1:b
			...
			for k in -c_2:c_2
				for l in -d_2:d_2
					e = K[k + c_2 + 1, l + d_2 + 1] * extend(M, i+k, j+l)
                    accumulator += e
				end
			end
...
function gaussian_kernel_1D(n; σ = 1)
	...
	l_half = (l-1) ÷ 2
	for i in 1:l
		a[i] = gauss(i-l_half-1, σ)
	end
	a = a ./ sum(a)
	return a
end

# ╔═╡ d89e15e2-5ac7-4baa-b99a-4040e44215e1
gaussian_kernel_1D(4; σ=1)
...
function with_gaussian_blur(image; σ=3, l=5)
	...
	for i in 1:b
		for j in 1:c
			a[i, j] = gauss(i-d-1, j-d-1; σ=σ)
		end
	end
	a = a ./ sum(a)

	new_image = convolve(image, a)
	return new_image
end
...
# ╔═╡ 9cadebf6-9a72-466f-a25f-990c61debf55
function c_bnw_float(c::AbstractRGB)
	equiv_float = mean(c.r + c.g + c.b)
	return equiv_float
end

# ╔═╡ 4cdae540-41f5-4ac9-8926-5bc29e9685c4
function b_n_w_matrix(image::AbstractArray)
	equiv_matrix = c_bnw_float.(image)
	return equiv_matrix
end

# ╔═╡ 9eeb876c-ee15-11ea-1794-d3ea79f47b75
function with_sobel_edge_detect(image::AbstractArray)
	b = c_bnw_float.(image)
...
# ╔═╡ 58892180-59d5-4ead-9146-61f3402b1d78
...
# ╔═╡ c76000df-656c-4a47-a5b7-1930f91382ce
sobel_cam_image = process_raw_camera_data(sobel_raw_camera_data)

# ╔═╡ 1bf94c00-ee19-11ea-0e3c-e12bc68d8e28
(with_sobel_edge_detect(sobel_cam_image))
