...
# ╔═╡ 93daa5e3-17eb-45a8-97ae-687fe32d193f
...
# ╔═╡ 2375a369-3fcc-47f5-833b-d4afa8abf103
function choose_2_agents(agents::Vector{Agent})
	if length(agents) < 2
		return nothing
	end
...
# ╔═╡ 309b1217-58b4-4135-a7a6-d3ad89e3d52d
function SIR_count(agents::Vector{Agent})
	...
	for i in eachindex(agents)
		if is_susceptible(agents[i])
			susceptible += 1
		elseif is_infected(agents[i])
			infected += 1
		else
			recovered += 1
		end
...
# ╔═╡ e0ec5d9a-e10f-43ff-8839-94333d0359b8
function get_R(sim::Vector{NamedTuple{(:S, :I, :R), Tuple{Int64, Int64, Int64}}})
	return [sim[i].R for i ∈ 1:length(sim)]
end
...
# ╔═╡ 28905a63-ce5d-4c16-b1a9-e7a8bfcf9dd0
function mean_S_all_t(simulations::Vector{<:NamedTuple})

	mean_S = []
	num_of_simulations = eachindex(simulations)
	num_of_time_steps = eachindex(first(simulations).S)
	
	for t in num_of_time_steps
		accumulator = []
		for i in num_of_simulations
			push!(accumulator, simulations[i].S[t])
		end
		push!(mean_S, mean(accumulator))
	end
...
# ╔═╡ e475f2d5-6060-4c4a-a091-c61d5411ee9c
function mean_I_all_t(simulations::Vector{<:NamedTuple})
...
function mean_R_all_t(simulations::Vector{<:NamedTuple})
	...
	mean_R = []
	num_of_simulations = eachindex(simulations)
	num_of_time_steps = eachindex(first(simulations).R)
	
	for t in num_of_time_steps
		accumulator = []
		for i in num_of_simulations
			push!(accumulator, simulations[i].R[t])
		end
		push!(mean_R, mean(accumulator))
	end
...
function sir_mean_plot(simulations::Vector{<:NamedTuple})
	mean_S = mean_S_all_t(simulations)
	mean_I = mean_I_all_t(simulations)
	mean_R = mean_R_all_t(simulations)

	p = plot()

	plot!(p, eachindex(first(simulations).S), mean_S, alpha=.5, label="susceptible")
	plot!(p, eachindex(first(simulations).I), mean_I, alpha=.5, label="infected")
	plot!(p, eachindex(first(simulations).R), mean_R, alpha=.5, label="recovered")
...
@bind p_infection_2 Slider(0.0001:0.0001:1, show_value=true)
...
# ╔═╡ 9e22fcf3-ca61-4749-8198-2bee3b9c644a
function std_S_all_t(simulations::Vector{<:NamedTuple})
...
# ╔═╡ 8e1d5dc4-7c66-4985-bfde-bed273c4e417
function std_I_all_t(simulations::Vector{<:NamedTuple})
...
# ╔═╡ 46e7bb08-df91-4c01-a5a3-cd3f714395ec
function std_R_all_t(simulations::Vector{<:NamedTuple})
...
# ╔═╡ 326139de-4b34-4aac-b115-33eae44edc02
@bind p_infection_3 Slider(0.0001:0.0001:1, show_value=true)
